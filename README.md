...
Before installing ChronoCode, ensure you have the following installed on your system:
- **Java 11+** (Required for the RefactoringMiner/JPype backend)
- **Python 3.10+** (Required for the analysis scripts)
- **Node.js & npm** (Required for the Vite frontend)
- **DesignitePy (DPy)**: You must manually download the DPy executable from [https://www.designite-tools.com/download](https://www.designite-tools.com/download). You will need to provide the path to this executable when running analyses or configuring settings.

## Installation & Running

ChronoCode provides two ways to run the application: using Docker (recommended for consistency) or via native scripts.

### Option 1: Docker (Recommended)

Using Docker completely bypasses the need to manually install Java, Python, and Node.js on your host machine.

1. Open your terminal in the ChronoCode directory.
2. Build and start the container using Docker Compose:
   ```bash
   docker-compose up --build
   ```
3. (Optional) If you want to analyze local repositories on your host machine, uncomment and update the volume mount in `docker-compose.yml` to map your local code folders into the container.

### Option 2: Native Installation

If you prefer running without Docker, ChronoCode provides native scripts to automate the Python virtual environment setup.
...
2. Run the installation script:
   ...
   ./install.sh
   ```
   *(Note: On Apple Silicon Macs, the script will automatically use Rosetta to create an x86_64 environment for JPype compatibility).*
...
**Windows**
1. Open PowerShell in the ChronoCode directory.
2. Run the installation script:
   ```powershell
   .\install.ps1
   ```
